...
- `GET/POST /api/me/family/` — связи владелец → член семьи (существующий `user_id`).
- `DELETE /api/me/family/<member_id>/`
- В истории болезней: поле **`subject_user_id`** — для кого запись (сам или привязанный профиль).
